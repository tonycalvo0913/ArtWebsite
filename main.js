...
//Should get information from Photoshop Art table
var Airtable = require('airtable');
var base = new Airtable({apiKey: 'keyB7EQxA9Lt2lvXE'}).base('appeG8kkwJsPViya8');

base('Photoshop Art').find('reclrZ2RzoXNlHrjJ', function(err, record) {
    if (err) { console.error(err); return; }
    console.log('Retrieved', record.id);
});

var listView = function(id, workname, rating) {
    return `<div class="col-sm-6">
    <div class="card mb-4 box-shadow">
          <h2><a href="?id=${id}">${workname}</a></h2>
          <div class="d-flex justify-content-between align-items-center">
  ...
  $.getJSON( airtable_list_url, function( data ) {
    ...
    $.each( data.records, function( index, val ) {
     //console.log(val.fields)
        var id = val.id;
        var fields = val.fields;
        var workname = fields["WorkName"];
        var rating = fields["Rating"];
        var itemHTML = listView(id, workname, rating);
        html.push(itemHTML);
    });
    html.push(`</div>`);
  // 3. Adds HTML for every item to our page
...
// Template that generates HTML for one item in our detail view, given the parameters passed in
var detailView = function(id, workname, picture, explanation, rating) {
  return `<div class="col-sm-12">
    <div class="card mb-4 box-shadow">
...
var getDataForId = function(id) {
  $.getJSON(`https://api.airtable.com/v0/appeG8kkwJsPViya8/Hand%20Drawings/${id}?api_key=keyB7EQxA9Lt2lvXE`, function( record ) {
    ...
    html.push(`<div class="row">`);
      ...
      var explanation = fields["Explanation"];
      var rating= fields["Rating"];
      var itemHTML = detailView(id, workname, picture, explanation, rating);
      html.push(itemHTML);
    html.push(`</div>`);
    $(".detail-view").append(html.join(""));
  });
